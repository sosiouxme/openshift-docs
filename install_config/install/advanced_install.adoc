...
However, this method should not be used to run the installer on one of the target hosts,
as the install typically restarts *docker* on the host, which would disrupt installer
container execution.
...
. An inventory. This can be a location inside the container (typically
mounted with a volume) with a path referenced via the `INVENTORY_FILE`
environment variable. Alternatively you can serve the inventory file from
a web server and use the `INVENTORY_URL` environment variable to fetch
it, or `DYNAMIC_SCRIPT_URL` to download an executable script that provides
a dynamic inventory.
. SSH keys so that Ansible can reach your hosts. These should be mounted
as a volume under *_/opt/app-root/src/.ssh_*.
. The playbook to run. This is set using the `PLAYBOOK_FILE` environment
variable, and typically refers to a playbook from the *openshift-ansible*
RPM, but can be any playbook available inside the container.

* Here is an example of how to run an install via *docker*. Note that
this must be run by a non-`root` user with access to *docker*.
+
----
$ docker run -t -u `id -u` \
       -v $HOME/.ssh/id_rsa:/opt/app-root/src/.ssh/id_rsa:Z \//<1>
       -v $HOME/ansible/hosts:/tmp/inventory:Z \//<2>
       -e INVENTORY_FILE=/tmp/inventory \
       -e PLAYBOOK_FILE=playbooks/byo/config.yml \//<3>
       -e OPTS="-v" \
       openshift/origin-ansible
----
<1> This mounts an SSH key that provides access to the target hosts. In
this example, the user's own default SSH key is used.
<2> This mounts in an inventory file (in this example, from the user's
home directory) for *Ansible* to use. On the next line `INVENTORY_FILE`
is set to the location where it is mounted.
<3> This `PLAYBOOK_FILE` specifies the standard BYO installer.

Here is a detailed explanation of the options used in the command above:

* `-t` allocates a TTY so that *Ansible* adds color to the output.
...
+
Note that the SSH key is mounted with the `:Z` flag: this
is required so that the container can read the SSH key
under its restricted SELinux context; this also means that
your original SSH key file will be re-labeled to something like
`system_u:object_r:container_file_t:s0:c113,c247`. For more details about
`:Z` please check the `docker-run(1)` man page. Please keep this in
mind when providing these volume mount specifications because this could
...
mount the *Ansible* inventory file into the container as
